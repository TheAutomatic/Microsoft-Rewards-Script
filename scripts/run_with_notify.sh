...
(
  tail -n 0 -F "$LOG_FILE" 2>/dev/null | while IFS= read -r line; do
    if [[ "$line" == *"Please approve login and select number:"* ]]; then
      ...
      [[ -n "$num" ]] || num="(unknown)"
      event_ts="$(date '+%F %T')"
      notify approval "[Rewards] ${num} 需要确认" "Authenticator 数字匹配：${num}\n当前时间：${event_ts}\n主机：${HOST_TAG}\n时区：${TZ_TAG}\n日志：${LOG_BASENAME}"
    fi
  done
) &
WATCHER_PID=$!

# Watchdog: detect stalled retry loops and force-stop hung run
WATCHDOG_ENABLED="${WATCHDOG_ENABLED:-1}"
WATCHDOG_CHECK_SEC="${WATCHDOG_CHECK_SEC:-60}"
WATCHDOG_STALL_SEC="${WATCHDOG_STALL_SEC:-360}"
WATCHDOG_SIGNATURE="${WATCHDOG_SIGNATURE:-Retrying search|locator\.fill: Timeout|SEARCH-BING\] Search attempt failed}"
WATCHDOG_PID=""

if [[ "$WATCHDOG_ENABLED" == "1" ]]; then
(
  while true; do
    sleep "$WATCHDOG_CHECK_SEC"

    node_pid="$(pgrep -f "node ./dist/index.js" | head -n1 || true)"
    [[ -n "$node_pid" ]] || continue

    now_epoch="$(date +%s)"
    log_mtime="$(stat -c %Y "$LOG_FILE" 2>/dev/null || echo "$now_epoch")"
    idle_sec=$((now_epoch - log_mtime))
    [[ "$idle_sec" -ge "$WATCHDOG_STALL_SEC" ]] || continue

    if tail -n 240 "$LOG_FILE" | grep -qE "$WATCHDOG_SIGNATURE"; then
      event_ts="$(date '+%F %T')"
      notify watchdog "[Rewards] 检测到疑似卡住，准备重启" "时间：${event_ts}\n主机：${HOST_TAG}\n日志：${LOG_BASENAME}\n特征：重试/超时后长时间无新日志\n空闲：${idle_sec}s\nPID：${node_pid}"
      ...
      notify watchdog "[Rewards] 已执行重启保护" "时间：${event_ts}\n动作：已终止卡住进程（pid=${node_pid}）\n说明：本次 run 将结束，交由下一次任务重启"
      break
    fi
  done
) &
